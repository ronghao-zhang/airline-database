...
Creat Traveler Data and Attributes
```{r}
raw_travlers <- randomNames(3000,return.complete.data = TRUE)
set.seed(2022341)
# attribute user_id
traveler_id <- sample(1000001:9999999,nrow(raw_travlers), replace = FALSE)
# attribute dob
...
final_travelers_data <- cbind(traveler_id,raw_travlers,traveler_dob,
  ...
  mutate(gender = replace(gender, weather == '1', 'F'), 
         gender = replace(gender, weather == '0', 'M')) %>% 
